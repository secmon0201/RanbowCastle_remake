...
机制,旧写法,新写法 (支持动画),默认动画
A1. 攻击特效,"<战斗触发:Attack, 公式>","<战斗触发:Attack, 公式, 动画ID>",1 (打击)
B1. 受击特效,"<战斗触发:Hit, 公式>","<战斗触发:Hit, 公式, 动画ID>",1 (打击)
A3. 亡语,"<战斗触发:Dead, 公式>","<战斗触发:Dead, 公式, 动画ID>",1 (打击)
A2. 蓄力释放,"<蓄力释放: ID, 公式>","<蓄力释放: ID, 公式, 动画ID>",1 (打击)
B3. 守护光环,"<守护光环: ID, 率, 公式>","<守护光环: ID, 率, 公式, 动画ID>",52 (支援)
C1. 状态交互,"<状态交互: ID, 公式, 移, 范围>","<状态交互: ID, 公式, 移, 范围, 动画ID>",自适应*
C2. 力场共鸣,"<力场共鸣: ID, 模, 公式, 移>","<力场共鸣: ID, 模, 公式, 移, 动画ID>",1 (打击)
C3. 溅射伤害,"<溅射伤害: 公式, 范围>","<溅射伤害: 公式, 范围, 动画ID>",1 (打击)
C7. 弹射(闪电),"<弹射伤害: 初, 递, 次, 限, 复, 模>","<弹射伤害: ..., 模式, 动画ID>",1 (打击)
C4. 斩杀,"<斩杀追击: 阈, 公式>","<斩杀追击: 阈, 公式, 动画ID>",1 (打击)
C5. 吸血,<技能吸血: 率>,"<技能吸血: 率, 动画ID>",46 (治疗)
(C1 自适应：Target=1, AllAllies=46, Self=52)

机制,标签写法,动画目标
A1/B1/A3 战斗触发,"<战斗触发:类型, 公式> (无动画参数)",无
B2. 受击蓄力,"<受击蓄力: 状态ID, 动画ID>",自己 (Target)
A2. 蓄力释放,"<蓄力释放: 状态ID, 公式, 动画ID>",敌人 (Target)
C 技能特效系列,(保持 v3.5.1 的动画参数写法),相应目标
...
### 1.2 插件特点
- **[New] 日志净化终极版**：彻底屏蔽所有被动触发产生的战斗文字日志，保留视觉反馈
- **[New] 全参数化调整**：所有延迟时间均可在插件参数中调整
- **[New] 新增蓄力系统**：受击蓄力和蓄力释放机制
- **[New] 新增守护光环**：伤害转移和守护机制
- **[New] 增强弹射伤害**：支持递减延迟的闪电链效果，第一跳使用初始公式，后续跳使用递推公式
...
**参数说明**:
- `状态ID`: 整数，代表蓄力状态的ID
- `释放公式`: 任意有效的 JavaScript 代码
  - 变量 `d` = 累计的伤害值
  - 变量 `a` = 自身
  - 变量 `b` = 敌人

**功能说明**:
攻击时，如果持有此状态且有蓄力值，则触发额外伤害。

**示例**:
<蓄力释放: 50, d * 2> //释放双倍累计伤害

#### 4.1.6 守护光环

**填写位置**: 角色/职业/装备/状态 备注

**格式**: `<守护光环: 状态ID, 承伤比例, 减免公式>`

**参数说明**:
- `状态ID`: 整数，代表守护状态的ID
- `承伤比例`: 0.0-1.0 之间的小数，代表转移伤害的比例
- `减免公式`: 任意有效的 JavaScript 代码，用于计算守护者实际承受的伤害
  - 变量 `damage` = 原伤害

**功能说明**:
当队友受伤时，如果持有此状态且活着，则分担伤害。

**示例**:
<守护光环: 60, 0.8, damage * 0.8> //坦克必须有60号状态。队友受伤时，80%的伤害转移给坦克，坦克实际承受转移量的80%
...
**格式**: `<弹射伤害: 初始公式, 递推公式, 次数, 上限, 重复?, 模式>`

**参数说明**:
- `初始公式`: 对**第一跳目标**的伤害（支持 eval）
- `递推公式`: 对**后续弹跳目标**的伤害（变量 `damage` 为上一次伤害）
- `次数`: 额外弹射次数
- `上限`: 单次最大伤害（0为不限）
- `重复`: `true`/`false`（是否允许弹回同一个人）
...
**示例**:
<弹射伤害: 500, damage * 0.8, 3, 0, false, Random> //首发500伤害，弹3次，第一跳500，后续每次衰减20%，不重复，随机目标
<弹射伤害: a.atk * 2, damage, 4, 9999, false, Order> //首发2倍攻击，弹4次，第一跳2倍攻击，后续不衰减，按站位顺序弹射
...
- **强制插队**: 使用 `this.forceAction(observer)` 确保协战执行
...
### v3.3.2 (Current)
* **[核心修复] 彻底屏蔽了所有被动触发（A1/B1/亡语等）产生的战斗文字日志**
* **[视觉优化] 完美保留伤害数字、回血数字、受击动画等视觉反馈**
* **[解决问题] 解决了“受击回蓝”等标签导致战斗信息刷屏的问题**
* **[增强] 弹射伤害修复**：正确应用初始公式和递推公式，第一跳使用初始公式，后续跳使用递推公式
* **[优化] 协战延迟参数化**：使用 `SynergyDelay` 参数控制协战延迟

### v3.3.0
* **[Refactor] 全参数化调整**：所有延迟时间均可在插件参数中调整，包括守护光环间隔、蓄力释放延迟、队友协战延迟、闪电链初始间隔和闪电链延迟递减
* **[Feature] 增强弹射伤害**：重新加入并增强了“弹射伤害(闪电链)”模块，支持递减延迟，每次弹射的间隔时间逐渐减少
* **[Core] 守护光环响应速度优化**：守护光环的触发机制得到优化，响应更快

### v3.0.0
 * 1. 移除了老旧的 processComplexFeatures 和全局 Map，大幅优化性能。
 * 2. “累计反击”和“伤害转移”现在完全融入被动模块，不再需要复杂的配置。
 * 3. 所有的能力（反击、守护、溅射）现在都支持从 角色/职业/装备/状态 中读取。
 *
 * ============================================================================
 * 一、累计反击 (Vengeance) - 新写法
 * ----------------------------------------------------------------------------
 * 逻辑拆分为“蓄力”和“释放”两部分，更加灵活。
 *
 * 1. 开启蓄力 (写在 状态/职业/装备 备注里):
 * <受击蓄力: 状态ID>
 * 说明：当持有此状态ID时，受到的伤害会被记录下来。
 *
 * 2. 释放反击 (写在 状态/职业/装备 备注里):
 * <蓄力释放: 状态ID, 释放公式>
 * 说明：攻击时，如果持有此状态且有蓄力值，则触发额外伤害。
 * 公式变量: d = 累计的伤害值, a = 自身, b = 敌人
 *
 * 例：<受击蓄力: 50>
 * <蓄力释放: 50, d * 2>  (释放双倍累计伤害)
 *
 ...
 * 二、伤害转移 (Guardian) - 新写法
 * ----------------------------------------------------------------------------
 * 这是一个“光环”效果。写在“坦克”的 状态/职业/装备 备注里。
 *
 * <守护光环: 状态ID, 承伤比例, 减免公式>
 * 说明：当队友受伤时，如果“坦克”持有此状态且活着，则分担伤害。
 *
 * 变量: damage = 原伤害
 *
 * 例：<守护光环: 60, 0.8, damage * 0.8>
 * (坦克必须有60号状态。队友受伤时，80%的伤害转移给坦克，坦克实际承受转移量的100%)
 *
 * ============================================================================
 */
